...
var ScalarObservable = (function (_super) {
    __extends$13(ScalarObservable, _super);
...
var EmptyObservable = (function (_super) {
    __extends$14(EmptyObservable, _super);
...
var ArrayObservable = (function (_super) {
    __extends$12(ArrayObservable, _super);
...
var InnerSubscriber = (function (_super) {
    __extends$17(InnerSubscriber, _super);
...
var DebounceTimeSubscriber = (function (_super) {
    __extends$18(DebounceTimeSubscriber, _super);
...
var LOCALSTORAGE_PREFIX = 'ObStore/';
var ObStoreItem = /** @class */ (function () {
    function ObStoreItem(name, _store, _opts) {
        if (_opts === void 0) { _opts = {}; }
        this.name = name;
        this._store = _store;
        this._opts = _opts;
        this._lastSetterId = 0;
        this._subject = new ReplaySubject_2(this._opts.bufferSize || 1);
        this.obs = this._subject.asObservable();
        this._handleOpts();
        // this._store[name] = this;
    }
    ObStoreItem.prototype.subscribe = function (next, error, complete) {
        return this.obs.subscribe(next, error, complete);
    };
    ObStoreItem.prototype.set = function (value) {
        return __awaiter(this, void 0, void 0, function () {
            var _this = this;
            var setterId, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        setterId = ++this._lastSetterId;
                        if (!this._transformer) return [3 /*break*/, 4];
                        this.params = __assign({}, this.params, value);
                        this._subjectParams.next(this.params);
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, this._transformer(this.params)];
                    case 2:
                        value = _a.sent();
                        return [3 /*break*/, 4];
                    case 3:
                    ...
                    case 4:
                        ...
                        if (setterId === this._lastSetterId) {
                            if (this._opts.localstorage) {
                                localStorage.setItem(LOCALSTORAGE_PREFIX + this.name, JSON.stringify(value));
                            }
                            this.value = value;
                            this._subject.next(this.value);
                            // TODO: сделать ленивый interval
                            if (this._opts.interval) {
                                setTimeout(function () { _this.repeat(); }, this._opts.interval);
                            }
                        }
                        return [2 /*return*/];
                }
    ...
    ObStoreItem.prototype.repeat = function () {
        this.set(this.params !== null ? this.params : this.value);
    };
    ObStoreItem.prototype.updateOpts = function (opts) {
        if (this._opts) {
            return;
        }
        this._opts = opts;
    ...
    ObStoreItem.prototype._handleOpts = function () {
        var _this = this;
        if (!this._opts) {
            return;
        }
        ...
        if (this._opts.depsOn) {
            this.obs = Observable_2.combineLatest(this._opts.depsOn.map(function (prop) {
                return Array.isArray(prop) ? _this._store[prop[0]][prop[1]] : _this._store[prop].obs;
            }));
            this._subject = null;
            if (this._transformer) {
                this.obs = this.obs.flatMap(function (data) { return __awaiter(_this, void 0, void 0, function () { return __generator(this, function (_a) {
                    return [2 /*return*/, this._transformer(data)];
                }); }); });
            }
            return;
        }
        ...
        if (this._transformer && !this._subjectParams) {
            this._subjectParams = new ReplaySubject_2(this._opts.bufferSize || 1);
        ...
        var storedValue = this._opts.localstorage
            ? JSON.parse(localStorage.getItem(LOCALSTORAGE_PREFIX + this.name) || 'null')
            : null;
        if (storedValue !== null) {
            this.set(storedValue);
        }
        else if (this._opts.default) {
            this.set(this._opts.default);
        }
        if (this._opts.interval) {
            setTimeout(function () { _this.repeat(); }, this._opts.interval);
        }
...
var ObStoreService = /** @class */ (function () {
    function ObStoreService(schema) {
        var _this = this;
        this.store = {};
        this._isSettedFromData = false;
        Object.keys(schema).forEach(function (key) {
            var opts = schema[key];
            var ObStore = _this.store[key];
            // tslint:disable-next-line:no-unused-expression
            _this.store[key] = new ObStoreItem(key, _this.store, opts);
            // if (!ObStore) new ObStoreItem<S>(key, opts);
            // else ObStore.updateOpts(opts);
        });
    }
    return ObStoreService;
}());
...
function ObStoredDecoratorFabric(ObStoreService) {
    return function (propName, opts) {
        return function (target, key) {
            var prop = propName || key;
            target[key] = ObStoreService.store[prop];
        };
    };
}

var ObStore = /** @class */ (function () {
    function ObStore(schema) {
        this._service = new ObStoreService(schema);
...
exports.ObStore = ObStore;

Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=ObStore.umd.js.map
